...
func main() {
	...
	//GET
	rtr.GET("/video/"+id, publicAccess)                             // viewing public video
	rtr.GET("/private/video/"+id, privateAccess)                    // viewing private video
	rtr.GET("/user/"+userName, creatorPage)                         // get the page of a crator
	rtr.GET("/user/"+userName+"/membersOnly", creatorPrivateVideos) // get the private videos of a creator
	rtr.GET("/home", home)                                          // homepage
	rtr.GET("/logoff", logoff)                                      // log the user off
	rtr.GET("/category/" +category, searchByCategory)
...
func debug() {
	...
	//db.Table("users").Create(&user{userName: "root", fullName: "root", email: "robdela@protonmail.com", password: "RmRbxGv4xLx$dnTz8Xve&m*^D8Xr#^99ebmf8h#AP", accessToken: "root"})
}
